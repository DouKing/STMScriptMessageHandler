...
- (void)prepareJsScript {
    ...
    [self _addJS2];
    [self _addJS3];
    [self registerMethod:kSTMNativeCallback handler:^(id  _Nonnull data, STMResponseCallback  _Nullable responseCallback) {
    ...
    NSString *jsScript = STM_JS_FUNC(
        function nativeCallback(methodName, info) {
            ...
            handler(info, function(data){
                %@.%@.callMethod('%@', {name: methodName, info: data}, null);
            });
        }
        , kSTMAppBridge, self.handlerName,
        kSTMAppBridge, self.handlerName, kSTMNativeCallback
    );
    [self _addJSScript:jsScript forMainFrameOnly:YES];
}
